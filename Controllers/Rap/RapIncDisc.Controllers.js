...
exports.RapIncDiscFill = async (req, res) => {
    ...
    jwt.verify(req.token, _tokenSecret, async (err, authData) => {
        if (err) {
            ...
            try {
                ...
                request = await request.execute('USP_FrmRapIncdFill');
...
exports.RapIncDiscSave = async (req, res) => {
    ...
    jwt.verify(req.token, _tokenSecret, async (err, authData) => {
        if (err) {
            ...
            try {
                ...
                request = await request.execute('USP_FrmRapIncdSave');
...
exports.RapIncDiscImport = async (req, res) => {
                ...
                for (let i = 0; i < RapArr[shapeIndex].length; i++) {
                    for (let j = 0; j < RapArr[shapeIndex][i].C_NAME.length; j++) {
                        try {
                            ...
                            request.input('C_NAME', sql.VarChar(20), RapArr[shapeIndex][i].C_NAME[j])
                            request.input('I_NAME', sql.VarChar(20), RapArr[shapeIndex][i].I_NAME)
                            ...
                            request = await request.execute('USP_FrmRapIncdImport');
...
exports.RapIncDiscExport = async (req, res) => {
    ...
    jwt.verify(req.token, _tokenSecret, async (err, authData) => {
        if (err) {
            ...
            try {
                ...
                request.input('I_CODE', sql.VarChar(5), req.body.I_CODE)

                request = await request.execute('USP_FrmRapIncdExport');
...
exports.RapIncDiscExcelDownload = async (req, res) => {
    ...
    if (req.body.shapeCount) {
        ...
        for (let shapeIndex = 0; shapeIndex < DataToExport.length; shapeIndex++) {
            ...
            if (RTYPE == 'H') {
                ...
                let xlsQua = Object.keys(DataToExport[shapeIndex][0].Data[0]).filter((item) => item.startsWith('I'))
    ...
    workbook.xlsx.writeBuffer().then(function (buffer) {
        ...
        res.writeHead(200, {
            ...
            'Content-disposition': 'attachment;filename=RapIncDisc.xlsx',
        }).end(xlsData);
    });
}
