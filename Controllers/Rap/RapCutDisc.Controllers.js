...
exports.RapCutDiscFill = async (req, res) => {
    ...
    jwt.verify(req.token, _tokenSecret, async (err, authData) => {
        if (err) {
            ...
            try {
                ...
                request.input('STYPE', sql.VarChar(5), req.body.STYPE)
                ...
                request.input('PRP', sql.Int, parseInt(req.body.PRP))
                request.input('FL_CODE', sql.Int, parseInt(req.body.FL_CODE))

                request = await request.execute('USP_FrmRapCutDFill');
...
exports.RapCutInsertDis = async (req, res) => {
    ...
    jwt.verify(req.token, _tokenSecret, async (err, authData) => {
        if (err) {
            ...
            try {
                ...
                request.input('RTYPE', sql.VarChar(3), req.body.RTYPE)
                request.input('TYP', sql.VarChar(10), req.body.TYP)

                request = await request.execute('USP_InsertDis');
...
exports.RapCutdSave = async (req, res) => {
    ...
    jwt.verify(req.token, _tokenSecret, async (err, authData) => {
        if (err) {
            ...
            try {
                ...
                request.input('STYPE', sql.VarChar(5), req.body.STYPE)
                request.input('RTYPE', sql.VarChar(2), req.body.RTYPE)
                request.input('SZ_CODE', sql.Decimal(10, 2), parseFloat(req.body.SZ_CODE))
                request.input('S_CODE', sql.VarChar(5), req.body.S_CODE)
                request.input('C_CODE', sql.Int, parseInt(req.body.C_CODE))
                request.input('PRP', sql.Int, parseInt(req.body.PRP))
                request.input('FL_CODE', sql.Int, parseInt(req.body.FL_CODE))
                ...
                request = await request.execute('USP_FrmRapCutdSave');
...
exports.RapCutdImport = async (req, res) => {
                ...
                for (let i = 0; i < RapArr[shapeIndex].length; i++) {
                    for (let j = 0; j < RapArr[shapeIndex][i].C_NAME.length; j++) {
                        try {
                            ...
                            request.input('STYPE', sql.VarChar(5), RapArr[shapeIndex][i].STYPE)
                            ...
                            request.input('PRP_NAME', sql.VarChar(10), RapArr[shapeIndex][i].PRP_NAME)
                            ...
                            // console.log('--------------------------------------')
                            request = await request.execute('USP_FrmRapCutdImport');
...
exports.RapCutdExport = async (req, res) => {
    try {
        // console.log(req.body.PRP)
        let request = new sql.Request()
        ...
        request.input('PRP', sql.VarChar(5), req.body.PRP)

        request = await request.execute('USP_FrmRapCutdExport');
...
exports.RapCutdExcelDownload = async (req, res) => {
    ...
    if (req.body.shapeCount) {
        ...
        for (let shapeIndex = 0; shapeIndex < DataToExport.length; shapeIndex++) {
            ...
            worksheet.getCell('B3').value = `${req.body.S_CODE}-${req.body.STYPE}-${shapeArr[shapeIndex]}`;
            ...
            if (RTYPE == 'H') {
                ...
                let xlsQua = Object.keys(DataToExport[shapeIndex][0].Data[0]).filter((item) => item.startsWith('CT'))
        ...
        var minutes = date_ob.getMinutes();
        var dateTime = year + "-" + month + "-" + day + " " + hours + ":" + minutes;
        var shape = req.body.SHAPE + "-" + req.body.STYPE + "-" + req.body.SheetName
        ...
        worksheet.getCell('B3').value = `${req.body.S_CODE}-${req.body.STYPE}-${req.body.SheetName}`;
        worksheet.getCell('B3').alignment = { horizontal: 'center', bgColor: { argb: 'FF00FF00' } };
        worksheet.getCell('B3').fill = {
            type: 'pattern',
            pattern: 'solid',
    ...
    workbook.xlsx.writeBuffer().then(function (buffer) {
        ...
        res.writeHead(200, {
            ...
            'Content-disposition': 'attachment;filename=RapCutDisc.xlsx',
        }).end(xlsData);
    });
