    ...
    jwt.verify(req.token, _tokenSecret, async (err, authData) => {
        if (err) {
            ...
            try {
                ...
                request.input('T_CARAT', sql.Numeric(10, 3), req.body.T_CARAT)
                request.input('COMP_CODE', sql.VarChar(10), req.body.COMP_CODE)
                request.input('YY', sql.Int, req.body.YY)
                request.input('MM', sql.Int, req.body.MM)
    ...
    jwt.verify(req.token, _tokenSecret, async (err, authData) => {
        if (err) {
            ...
            try {
                ...
                if (req.body.F_DATE) { request.input('F_DATE', sql.DateTime2, new Date(req.body.F_DATE)) }
                if (req.body.T_DATE) { request.input('T_DATE', sql.DateTime2, new Date(req.body.T_DATE)) }
                request.input('ISPASS', sql.Bit, req.body.ISPASS)
...
exports.TendarWinSheetUnmatch = async (req, res) => {
    ...
    columnZ.width = 5.60;
    columnAB.width = 5.60;
    columnAA.width = 10.14;
    columnAC.width = 5.71;
    columnAD.width = 26.86;
    ...
    mergeCellsBasedOnCondition(worksheet1, 'U', 21);
    mergeCellsBasedOnCondition(worksheet1, 'V', 22);
    mergeCellsBasedOnCondition(worksheet1, 'W', 23);
    mergeCellsBasedOnCondition(worksheet1, 'X', 24);
    mergeCellsBasedOnCondition(worksheet1, 'Y', 25);
    mergeCellsBasedOnCondition(worksheet1, 'Z', 26);
    mergeCellsBasedOnCondition(worksheet1, 'AA', 27);
    mergeCellsBasedOnCondition(worksheet1, 'AB', 28);
    ...
    worksheet1.eachRow({ includeEmpty: false }, (row, rowNumber) => {
        if (rowNumber > 7) {
            if (value != row.getCell(`O`).value) {
                value = row.getCell(`O`).value
                SumOfvalue += value
            }

            if (valueOfO != row.getCell(`P`).value) {
                valueOfO = row.getCell(`P`).value
                SumOfO += valueOfO
            }

            if (valueOfR != row.getCell(`S`).value) {
                valueOfR = row.getCell(`S`).value
                SumOfR += valueOfR
            }
        }
    })
    ...
    worksheet1.mergeCells(`P${rowNumber}:P${rowNumber + 1}`);
    worksheet1.mergeCells(`S${rowNumber}:S${rowNumber + 1}`);
    worksheet1.getCell(`D${rowNumber}`).fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'f8ff2b' }
    ...
    worksheet1.getCell(`P${rowNumber}`).fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'f8ff2b' }
    };
    worksheet1.getCell(`S${rowNumber}`).fill = {
        type: 'pattern',
        pattern: 'solid',
        fgColor: { argb: 'f8ff2b' }
    };
    worksheet1.getCell(`D${rowNumber}`).value = HeaderData[0].I_CARAT;
    worksheet1.getCell(`O${rowNumber}`).value = SumOfvalue.toFixed(0);
    worksheet1.getCell(`P${rowNumber}`).value = SumOfO.toFixed(0);
    worksheet1.getCell(`S${rowNumber}`).value = SumOfR.toFixed(0);
    ...
    worksheet1.mergeCells('D2:T6');
    worksheet1.getCell('D2').value = HeaderData[0].HEADER
    worksheet1.getCell('D2').font = { size: 48, bold: true }

    worksheet1.mergeCells('U2:AD6');
    worksheet1.getCell('U2').value = 'COLOR MACHINE COMMENT'
    worksheet1.getCell('U2').font = { size: 24, bold: true }
    worksheet1.getCell('U2').border = {
        top: { style: 'thin', color: { argb: 'FF000000' } },
        left: { style: 'thin', color: { argb: 'FF000000' } },
        bottom: { style: 'thin', color: { argb: 'FF000000' } },
        right: { style: 'thin', color: { argb: 'FF000000' } },
